...
/** Represents one product slot inside a combo/deal */
interface ComboSlot {
    id: string;
    slotName: string;
    quantity: number;
...
export const ComboBuilderStep: React.FC = () => {
    ...
    const comboSlots: ComboSlot[] = (formData as any).__comboSlots || [];
    const setComboSlots = (slots: ComboSlot[]) => {
        (formData as any).__comboSlots = slots;
        // Force re-render by updating a tracked field
                    ...
                    <div className="w-64 flex-shrink-0 space-y-2">
                        {comboSlots.map((slot, idx) => (
                            <button
                                ...
                                className={cn(
                                    "w-full flex items-center justify-between p-3.5 rounded-xl border-2 text-left transition-all group",
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <span className={cn(
                                        ...
                                        slot.isConfigured ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"
                                    )}>
                                        {slot.isConfigured ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : idx + 1}
                                    </span>
                                    <div>
                                        ...
                                        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                                            Qty: {slot.quantity} • {slot.isConfigured ? 'Ready' : 'Not configured'}
                                        </span>
                                    </div>
                                </div>
                                <button
                                    onClick={(e) => { e.stopPropagation(); removeSlot(slot.id); }}
                                    className="p-1 hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                                >
                        ...
                        <button
                            onClick={addSlot}
                            className="w-full flex items-center justify-center gap-2 p-3.5 border-2 border-dashed border-slate-200 hover:border-slate-400 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-wider transition-all hover:bg-slate-50"
                        >
                        ...
                        {comboSlots.length === 0 && (
                            <div className="text-center py-6 px-4">
                                ...
                                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                                    Add product slots to build your combo
                                </p>
                            </div>
                        )}
                    </div>

                    {/* Right: Active Slot Configuration */}
                        ...
                        {activeSlot ? (
                            ...
                            <div className="flex items-center justify-center h-64 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                                <div className="text-center">
                                    <ArrowRight className="w-6 h-6 text-slate-300 mx-auto mb-2 -rotate-180" />
